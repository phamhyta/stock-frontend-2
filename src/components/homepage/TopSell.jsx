...
import { topBanNuocNgoai, topGiamGia } from '../../common/api';
const TopSell = (props) => {
  const { mode } = props;
  const [datas, setDatas] = useState([]);
  useEffect(() => {
    const fetch = async () => {
      const res = await topBanNuocNgoai({ mode });
      if (res.code === 200) {
        setDatas(res.data);
      }
    };
  ...
  return (
    <div className="w-80">
      <div className="title">Top khối ngoại bán ròng</div>
      ...
      {datas.map((data) => (
        <div className="flex mt-4 w-full">
          ...
          <div className="w-1/4 font-semibold text-sm mr-4 my-auto flex justify-end items-end  text-red-600">
